...
//预处理（带错误检测）
void pretreatment(char* getfilepath, char* putfilepath, Statistics* stats, FILE* output_file) {
    FILE* yuan = NULL;
    FILE* yuchli = NULL;
    char ch;
    int line = 1, column = 1;
...
//词法分析（带错误检测和恢复）
void lexical_analysis(char* getfilepath, char* putfilepath, Statistics* stats, FILE* output_file) {
    fprintf(output_file, "开始词法分析：\n");
    // 打开文件
    FILE* cifa = NULL;
    FILE* yuchli = NULL;
    char ch;
    ...
    fprintf(output_file, "词法分析完成\n");
    fclose(yuchli);
    fclose(cifa);
}

// 输出统计结果
void print_statistics(Statistics* stats, FILE* output_file) {
    fprintf(output_file, "\n========== 统计结果 ==========\n");
    fprintf(output_file, "总行数: %d\n", stats->total_lines);
    fprintf(output_file, "语句行数: %d\n", stats->statement_lines);
    fprintf(output_file, "关键字个数: %d\n", stats->keyword_count);
    fprintf(output_file, "标识符个数: %d\n", stats->identifier_count);
    fprintf(output_file, "常数个数: %d\n", stats->constant_count);
    fprintf(output_file, "运算符个数: %d\n", stats->operator_count);
    fprintf(output_file, "分隔符个数: %d\n", stats->delimiter_count);
    fprintf(output_file, "字符总数: %d\n", stats->total_chars);
    fprintf(output_file, "词法错误总数: %d\n", stats->error_count);
    fprintf(output_file, "==============================\n");
}
...
int main() {
    ...
    if (fopen_s(&output_file, output, "w") != 0) {
        printf("输出文件打开失败！");
        return 1;
    }
    
    fprintf(output_file, "========== C语言词法分析器（带错误检测和恢复） ==========\n");
    fprintf(output_file, "正在分析文件: %s\n", yuan);
    fprintf(output_file, "====================================================\n\n");
    
    pretreatment(yuan, yuchli, &stats, output_file);
    lexical_analysis(yuchli, cifa, &stats, output_file);
